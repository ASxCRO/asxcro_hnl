...
public:
	...
	TextTables(char horizontal = '-', char vertical = '|', char corner = '+') :
...
inline
std::ostream & operator<<(std::ostream & stream, TextTables const & table)
{
	table.setup();
	stream << table.ruler() << "\n\t";
	for (auto rowIterator = table.rows().begin(); rowIterator != table.rows().end(); ++rowIterator) {
		TextTables::Row const & row = *rowIterator;
		stream << table.vertical();
		for (unsigned i = 0; i < row.size(); ++i) {
			auto alignment = table.alignment(i) == TextTables::Alignment::LEFT ? std::left : std::right;
